 ...
 *
 * The x86_64 twin of build/x86-tests/badbuf-x86.c, kept line-for-line
 * comparable with it: same cases, same exit codes, same output format, and
 * the same hand-rolled number formatting even though this one is linked
 * against the CRT (build/x64-tests/build.sh does not pass -nostdlib).  A
 * divergence between the two runs is then a WoW64-versus-ARM64EC difference
 * and nothing else.  Faults are caught with a VECTORED handler rather than
 * __try for the same reason (clang has no SEH for i386, so the 32-bit twin
 * has no choice and this one follows it).
 *
...
/* formatted by hand, to stay identical to the i386 twin. */
...
static void write_ptr(const void *value)
{
    char buf[24];
    ULONG_PTR v = (ULONG_PTR)value;
    DWORD i = sizeof(buf);

    if (!v) buf[--i] = '0';
    else while (v && i > 0) { buf[--i] = "0123456789abcdef"[v & 15]; v >>= 4; }
    WRITE_LINE("0x");
    write_log(buf + i, sizeof(buf) - i);
}
...
static volatile ULONG_PTR g_fault_addr;
...
static LONG CALLBACK veh_handler(EXCEPTION_POINTERS *ep)
{
    ...
    if (ep->ExceptionRecord->NumberParameters >= 2)
        g_fault_addr = (ULONG_PTR)ep->ExceptionRecord->ExceptionInformation[1];
    ...
    if (code == EXCEPTION_ACCESS_VIOLATION)
    {
        ...
        addr = (void *)(g_fault_addr & ~(ULONG_PTR)0xfff);
...
int main(void)
{
    static const WCHAR name[] = { 'b','a','d','b','u','f','-','x','6','4','.','t','m','p',0 };
