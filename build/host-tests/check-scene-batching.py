...
r=Path.cwd();s=(r/'research/dxmt/src/d3d9/d3d9_device.cpp').read_text()
def body(name):
...
'''
p=r/'.xtool/ml1250-batching.cpp';p.write_text(stub+'HRESULT Device::BeginScene()'+body('BeginScene')+'\nHRESULT Device::EndScene()'+body('EndScene')+main)
o=r/'.xtool/ml1250-batching';subprocess.run(['g++','-std=c++20','-O1','-fsanitize=address,undefined',str(p),'-o',str(o)],check=True);subprocess.run([str(o)],check=True)
